...
## vLLM编译构建调用链  
vLLM项目底层算子等部分是cpp实现(CUDA)，系统核心部分用python搭建，因此构建过程需要python的setup.py搭配ext机制完成整个编译调用链。   

```shell
uv pip install -e .
│
├─ PEP 517: pyproject.toml → build-backend = "setuptools.build_meta"
│   构建时先安装: cmake, ninja, torch, setuptools
│
└─ python setup.py develop
    │
    ├─ DevelopAndGenerateGrpc.run()
    │   ├─ compile_grpc_protos()          # 生成 pb2 文件
    │   └─ develop.run()                  # setuptools editable install 逻辑
    │
    ├─ BuildPyAndGenerateGrpc.run()       # 打包 vllm/ Python 源码
    │
    └─ cmake_build_ext.build_extensions()
        │
        ├─ [Step 1] cmake configure
        │   ├─ find_package(Torch) → 激活 CUDA language
        │   ├─ FetchContent: cutlass, vllm-flash-attn, triton_kernels, flashmla
        │   ├─ execute_process: generate_kernels.py (Marlin)
        │   ├─ execute_process: generate.py (Machete)
        │   └─ define_extension_target(_C, _moe_C, cumem_allocator, fa2, fa3 ...)
        │
        ├─ [Step 2] cmake --build -j=N
        │   └─ ninja 并行编译所有 .cu/.cpp → .abi3.so
        │       (nvcc 编译 CUDA kernel, g++ 编译绑定代码)
        │
        └─ [Step 3] cmake --install --component <name>  (每个 ext 各一次)
            └─ 将 _C.abi3.so 等复制到 vllm/ 源码目录内
